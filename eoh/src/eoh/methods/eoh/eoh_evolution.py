...
class Evolution():
    ...
    def __init__(self, api_endpoint, api_key, model_LLM,llm_use_local,llm_local_url, debug_mode,prompts, **kwargs):
        ...
        self.debug_pause = os.getenv("EOH_DEBUG_PAUSE", "false").lower() in {"1", "true", "yes", "on"}
    ...
    def _format_guardrails(self):
        return (
            "Return exactly two parts and nothing else.\n"
            "Part 1: on the first line, write exactly one short algorithm description inside a single pair of braces.\n"
            "Part 2: immediately after that, write only raw Python code for the function.\n"
            "Do not include analysis, reasoning, thinking steps, <think> tags, markdown fences, bullet points, or any text before the opening brace.\n"
            "Do not explain the code.\n"
            "The code must define the required function name exactly and must end by returning the required output(s)."
        )

    def _strip_reasoning(self, response):
        cleaned = re.sub(r"<think>.*?</think>\s*", "", response, flags=re.DOTALL | re.IGNORECASE)
        cleaned = re.sub(r"```(?:python)?", "", cleaned, flags=re.IGNORECASE)
        cleaned = cleaned.replace("```", "")
        return cleaned.strip()

    def _debug_wait(self):
        if self.debug_pause:
