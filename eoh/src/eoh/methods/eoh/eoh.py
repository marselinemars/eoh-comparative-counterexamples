    ...
    def run(self):
        ...
        if self.use_seed:
            ...
            if self.load_pop:  # load population from files
                ...
                if len(population) == 0:
                    raise RuntimeError(
                        "Initial population is empty. All generated candidates failed evaluation. "
                        "Re-run with EOH_DEBUG=true to inspect the model outputs."
                    )
        ...
        for pop in range(n_start, self.n_pop):  
            ...
            if len(population) == 0:
                raise RuntimeError(
                    f"Population became empty after generation {pop + 1}. "
                    "All offspring in this round failed evaluation."
                )
