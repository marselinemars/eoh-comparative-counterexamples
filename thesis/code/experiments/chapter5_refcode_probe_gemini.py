"""
thesis/code/experiments/chapter5_refcode_probe_gemini.py

6-call Chapter-5 probe on Gemini 2.5 Pro at reasoning_effort=
"medium" — reproduces the author's prior exploratory setup that
produced 14/14 sanitization and median delta_step +0.37.

Production default in llm_client.py stays at "low"; this probe
overrides reasoning_effort via an explicit call_llm argument,
bypassing run_single_proposal (which hardcodes "low"). The body
below mirrors runner.py::run_single_proposal with the reasoning_
effort threaded through.

Usage:
    python thesis/code/experiments/chapter5_refcode_probe_gemini.py
"""
...
OUTPUT_DIR = (
    ...
    / "thesis"
    / "results"
    / "chapter5_refcode_probe_gemini_medium_2026_04_23"
)
...
LATENCY_ABORT_SECONDS = 180.0
COST_ABORT_USD = 0.30
INTER_CALL_SLEEP_SECONDS = 2.0
MAX_CONSECUTIVE_FAILURES = 3

# Gemini 2.5 Pro pricing, USD per 1M tokens (input/output). Reasoning
...
def _cost(usage: Dict[str, Any]) -> float:
    ...
    # Gemini bills reasoning at the output rate.
    return (
...
def _score_on_split(
    ...
    for inst in split["instances"]:
        ...
        per.append(b)
    arr = np.array(per)
    return {"per_instance": per, "mean": float(arr.mean())}


def _baseline_on_split(
    module, h_hash: str, split_name: str, cache: ScoreCache
) -> Dict[str, Any]:
    split = load_split(split_name)
    per: List[float] = []
    for inst in split["instances"]:
...
def _run_one_proposal(
    ...
    *,
    ...
    """Mirrors runner.run_single_proposal with reasoning_effort
    ...
    if strategy_name in STOCHASTIC_STRATEGY_NAMES:
        ...
        raise RuntimeError(f"unclassified strategy: {strategy_name}")
...
def _summarize(
    record: Dict[str, Any], latency: float, prompt_length_chars: int
) -> Dict[str, Any]:
...
def main() -> int:
    ...
    print("=" * 72)
    print("Chapter 5 refcode probe on Gemini 2.5 Pro (medium)")
    print("=" * 72)
    ...
    for i, strategy_name in enumerate(STRATEGIES_ORDER):
        ...
        print(f"\n[call {i + 1}/6] strategy={strategy_name}")
        ...
        row = _summarize(record, latency, prompt_length)
        rows.append(row)
        ...
        if latency > LATENCY_ABORT_SECONDS:
            stopped = True
            stop_reason = (
                f"{strategy_name} latency {latency:.1f}s "
    ...
    for r in rows:
        ...
        print(
            ...
            f"{str(r['response_mentions_instance_details'])[:1]:>3} "
            f"{str(r['response_mentions_reference'])[:1]:>3}"
        )

    # --- Cross-strategy aggregates --------------------------------------
    n = len(rows)
    ...
    for r in rows:
        ...
        print(f"- {r['strategy_name']} (d_step={r.get('delta_step'):+.2f})")
    ...
    cat_C = stopped or n_ok <= 1 or all_catastrophic
    cat_A = (
        not cat_C
        and n_ok >= 4
        and near_parity >= 2
    ...
    print(
        ...
        f"    batch_cost<$50? {batch_cost < 50.0} (${batch_cost:.2f})\n"
        f"    batch_wall<12h? {batch_wall < 12.0} ({batch_wall:.2f}h)\n"
