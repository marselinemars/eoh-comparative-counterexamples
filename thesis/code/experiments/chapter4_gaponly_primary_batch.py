"""
thesis/code/experiments/chapter4_gaponly_primary_batch.py

E2 (gap-only) primary batch driver — chapter 4 §4.5.2 control
...
(chapter4_gaponly.batch_runner skips any (set, seed) pair whose
JSON already exists).
...
reused); per-call LLM seeds are fresh under the ch4gaponly:
namespace.

Usage:
    python -m thesis.code.experiments.chapter4_gaponly_primary_batch
"""
...
def main() -> int:
    ...
    print("=" * 72)
    print(
        "CHAPTER 4 §4.5.2 — E2 GAP-ONLY primary batch "
    ...
    print("=" * 72)
    print(
        f"E2 batch done: {summary['n_calls_this_run']} calls this "
