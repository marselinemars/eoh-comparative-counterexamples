"""
thesis/code/chapter4_noref/batch_runner.py

E1 (no-reference) batch driver. Iterates (set_index 0..19,
seed_index 0..2) on chapter-5's stratified_representative L1 cell
to produce 60 sanitization-ok proposals matched-paired to chapter
5 by (set_index, seed_index). The CounterexampleSet at each
set_index is bit-identical to chapter-5's (chapter-5's set_seed
function is reused unchanged). The per-call LLM seed is fresh
under the `ch4noref:` namespace per design doc §6.2.
...
thesis/code/chapter4_noref/prompt_builder.build_prompt.

Resumable: per-call provenance JSONs at
thesis/results/chapter4_noref_primary_batch_gemini/set{NN}_seed{N}.json;
...
from thesis.code.chapter4_noref.prompt_builder import build_prompt
...
# Cell coordinates locked per design doc §3.3 / §3.4.
...
# Seed namespace prefix (design doc §6.2; decisions-log entry
# "ch4noref: and ch4gaponly: seed namespaces introduced").
SEED_NAMESPACE = "ch4noref"


def ch4noref_llm_seed(set_index: int, seed_index: int) -> int:
    """Per-call LLM seed under the ch4noref namespace. Independent
    of chapter-5's llm_seed (which uses the ch5:llm: prefix); the
    CounterexampleSet draw is still chapter-5's (set_seed reused)
    so the (set_index, seed_index) pair indexes matched calls
    across full / gap-only / no-reference cells."""
    return int(
        hashlib.sha256(
...
def run_single_proposal_noref(
    ...
    *,
    ...
    """Run one E1 proposal end-to-end and persist the provenance
    record. Parallels chapter5.runner.run_single_proposal but uses
    the E1 prompt builder and the ch4noref: seed namespace.
    """
    started_at = _utcnow_iso()

    # CounterexampleSet draw: reuse chapter-5's stratified_representative
    # with chapter-5's set_seed → bit-identical sets across cells.
    ...
    # Per-call LLM seed in the ch4noref namespace.
    derived_llm_seed = ch4noref_llm_seed(set_index, seed_index)

    # E1 prompt: no reference code, no gap, locked task instruction.
    prompt = build_prompt(
    ...
    record: Dict[str, Any] = {
        "cell_id": "chapter4_noref",
        "cell_description": (
            "E1 no-reference control; design doc §4.5.1. "
            "Stratified_representative @ L1 @ k=4. "
            "Reference source and gap_bins withheld; "
            "locked E1 task-instruction wording."
        ),
...
def run_noref_batch(
    ...
    *,
    ...
    """Run the E1 (no-reference) primary batch: 20 sets × 3 seeds
